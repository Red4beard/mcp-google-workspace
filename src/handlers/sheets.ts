...
function parseA1Range(range: string): { startRowIndex: number; endRowIndex: number; startColumnIndex: number; endColumnIndex: number } {
...
export async function handleSheetsFormatCells(
  sheets: sheets_v4.Sheets,
  params: {
  ...
  if (params.textColor) {
    format.textFormat = { ...(format.textFormat as object ?? {}), foregroundColor: hexToRgb(params.textColor) };
    fields.push("userEnteredFormat.textFormat.foregroundColor");
  }
  if (params.bold !== undefined) {
    format.textFormat = { ...(format.textFormat as object ?? {}), bold: params.bold };
    fields.push("userEnteredFormat.textFormat.bold");
  }
  if (params.fontSize !== undefined) {
    format.textFormat = { ...(format.textFormat as object ?? {}), fontSize: params.fontSize };
    fields.push("userEnteredFormat.textFormat.fontSize");
  }
  if (fields.length === 0) return "No formatting options provided.";

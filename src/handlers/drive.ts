...
export async function handleDriveSearch(
  drive: drive_v3.Drive,
  params: { query: string; folderId?: string; mimeType?: string }
): Promise<string> {
  ...
  const files = res.data.files ?? [];
  if (files.length === 0) return `No files found matching "${params.query}".`;
  return files.map((f) => `${f.name} — id: ${f.id}\n  ${f.webViewLink}`).join("\n");
}

export async function handleDriveGet(
  drive: drive_v3.Drive,
...
export async function handleDriveUpload(
  drive: drive_v3.Drive,
  params: { name: string; content: string; mimeType: string; parentId?: string }
): Promise<string> {
...
export async function handleDriveShare(
  drive: drive_v3.Drive,
  params: { fileId: string; email: string; role: string }
): Promise<string> {
...
export async function handleDriveCopy(
  drive: drive_v3.Drive,
  params: { fileId: string; newName?: string; parentId?: string }
): Promise<string> {
